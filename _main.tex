...
\textbf{Poisson distribution} \(\text{Pois}(\lambda)\)
\[
...
\chapter{Fundamentals Concepts}\label{fundamentals}}
...
\begin{definition}
In the frequentist world, the long run relative frequency of an event is the \textbf{probability} of that event.
\end{definition}
...
\begin{definition}
The maximum likelihood estimator of \(\theta\), \(\hat{\theta}\), is the value such that \(\hat{\theta} = \mathop{\mathrm{arg\,max}}_{\theta} \pi(Y\mid \theta)\).
\end{definition}
...
It is important to stress that the confidence interval (CI) \([L_n(Y), U_n(Y)]\) is a random quantity and the parameter \(\theta\) is fixed (non-random) in the frequentist paradigm! The correct interpretation of CIs is that if we construct many confidence intervals from repeated random samples, \(100(1-\alpha)\%\) of these intervals would contain the true parameter \(\theta\). It does \emph{not} mean that a particular interval contains the true value of \(\theta\) with probability \(1-\alpha\). Note that the CI interpretation is based on the previous definition of the probability of an event and this is why CIs based inference are considered to be frequentist.
...
\begin{definition}
In the Bayesian world, the subjective belief of an event is the \textbf{probability} of that event.
\end{definition}
...
\def\labelenumi{\arabic{enumi}.}
\item
  The marginal density function for \(X\) is \(\pi(x) = \int \pi(x,y)dy\).
\item
  The conditional density function of \(Y\mid X\) is \(\pi(y\mid x) = \pi(x,y)/\pi(x)\), although it is actually the density function of \(Y\mid X = x\). If \(X\) and \(Y\) are independent \(\pi(y \mid x) = \pi(y)\).
\item
  We can factorise a joint density function in different ways since \(\pi(x,y) = \pi(x)\pi(y\mid x) = \pi(x\mid y)\pi(y)\).
\item
...
\begin{definition}
Let \(\pi(y_1, \ldots, y_N)\) be the joint density of \(Y_1, \ldots, Y_N\). If \(\pi(y_1, \ldots, y_N) = \pi(y_{\sigma_1}, \ldots, y_{\sigma_N})\) for any permutations \(\sigma\) of \(\{1, \ldots, N\}\), then \(Y_1, \ldots, Y_N\) are \textbf{exchangeable}.
\end{definition}
...
\def\labelenumi{\arabic{enumi}.}
\item
  If \(\theta \in \mathbb{R}\) has density \(\pi(\theta) \propto\exp(-\theta^2/2)\) then \(\theta \sim N(0,1)\)
\item
  If \(\theta\in \{1,2,3\}\) with density (or proabibilty mass function) \(\pi(1)\propto 2, \pi(2) \propto 4, \pi(3) \propto 7\), then since \(\pi(1)+\pi(2)+\pi(3) = 1\), it must be the case \(\pi(1) = 2/13,\pi(2) = 4/13,\pi(3) = 7/13\).
\end{enumerate}
...
\textbf{Medium}. Simulate the value of \(\alpha_i\) and use the \texttt{cumprod} command to plot a trajectory of the market index over 100 days. Use a for loop to repeat this 100 times and investigate the distribution of the value of the stock market on days 50 and 100.
...
\item
  One way to improve the quality of these generators is to shuffle the sequence generated. Generate two sequences \(X\) and \(Y\) from two different LCGs, and report the shuffled sequence \(Z_j = X_{Y_j}\). For the sequence \(Y\) use the values \(a = 5\), \(c = 1\), \(m = 8\) and \(Y_0 = 2\).
\item
...
\begin{verbatim}
##  [1] -0.587508319  0.898055935  0.407552768 -0.830248269  0.759832296 -0.930477934
##  [7] -0.263275054  0.661190481  0.002115915 -0.007060681
\end{verbatim}
...
\includegraphics{_main_files/figure-latex/prior-posterior-grid2-1.pdf}
...
\begin{example}[Binomial likelihood]
\protect\hypertarget{exm:binom}{}\label{exm:binom}A social media company wants to determine how many of its users are bots. A software engineer collects a random sample of 200 accounts and finds that three are bots. Assuming that any two accounts being a bot are independent of one another, she decides to model the outcome as \(Y\mid \theta \sim \text{Bin}(200,\theta)\), and the observation is \(y = 3\).
...
\textbf{Likelihood function} \(\pi({y}\mid \theta)\). The Binomial likelihood function is given by
\[
...
\textbf{Posterior distribution} \(\pi(\theta \mid {y})\). We can now derive the posterior distribution up to proportionality
\[
...
We could consider reporting point estimates of \(\theta\). To give a specific estimate of \(\theta\), we may use \textbf{posterior mean}, i.e.~\(\mathbb{E}(\theta\mid y) = \int \theta \, \pi(\theta \mid y) d\theta\), which also requires knowing \(\pi(\theta\mid y)\) exactly in order to compute it exactly, or \textbf{posterior mode}, defined as
\[
...
\]
The posterior mode is also known as \textbf{maximum a posteriori (MAP)} estimate. If \(\pi(\theta) \propto 1\), then \(\hat{\theta}_{\text{MAP}}(y) = \hat{\theta}_{\text{MLE}}(y)= \mathop{\mathrm{arg\,max}}_{\theta \in \Theta} \pi( y \mid \theta).\) Another ideal property is that finding MAP does not require knowing the posterior exactly.
...
\begin{definition}
An interval \([l,u]\) is called a \textbf{credible interval} at level \(1-\alpha\), \(\alpha \in (0,1)\), for a random variable \(\theta \in \mathbb{R}\) if
\[
...
\hypertarget{conjugate-prior-and-posterior-analysis}{%
\section{Conjugate Prior and Posterior Analysis}\label{conjugate-prior-and-posterior-analysis}}
...
\begin{example}[Exponential likelihood]
...
\]
Note that the mean of the prior distribution is \(\mathbb{E}(\lambda) = \alpha/\beta\) and the MLE for \(\lambda\) is \(\hat{\lambda}_{\text{MLE}} = \frac{n}{\sum_{i=1}^ny_i}\). We can interpret the posterior mean as a weighted average of prior mean and the MLE by noticing
\[
...
\textbf{Posterior distribution} Using the formula we obtained before, the posterior distribution is \(\textrm{Gamma}(11, 95 + \beta)\). The posterior mean months until a claim is \(\frac{11}{95 + \beta}\).
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot.distributions}\NormalTok{(}\DecValTok{150}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\]
and the posterior variance is \(b^2\). Note that we have again that the posterior mean is a weighted average between the prior mean \(\mu_0\) and the MLE for \(\mu\), \(\hat{\mu}_{\text{MLE}} = \overline{y} = \sum y_i/N\). If we have either abundant data (\(N \rightarrow \infty\)) or a vague prior \(\sigma^2_0 \rightarrow \infty\), the weight on MLE increases to \(1\) and we then have the posterior mean \(\mathbb{E}(\mu\mid \boldsymbol{y}) \rightarrow \sum y_i/N\).
...
Finally, let's consider constructing (equal-tailed) credible intervals at level \(1-\alpha\). We need to find \(l,u \in \mathbb{R}\) such that
\[
...
\[
\hat{\beta}_{\text{ridge}} = (X^{\top}X+\lambda I)^{-1}X^{\top}Y = \mathop{\mathrm{arg\,min}}_{\beta \in \mathbb{R}^p} \Big\{\|Y-X\beta\|_2^2 + \lambda \|\beta\|_2^2\Big\},
\]
...
\]
Setting this equal to 0 gives us the maximum at \(x = \frac{3}{10}\). This means we can set \(c = \pi(3/10) = \frac{1}{B(4, 8)} \frac{3^7 7^{10}}{10^{10}}\). Our rejection sampling algorithm is therefore
...
\includegraphics{_main_files/figure-latex/unnamed-chunk-43-3.pdf}
...
\includegraphics{_main_files/figure-latex/unnamed-chunk-43-4.pdf}
...
\includegraphics{_main_files/figure-latex/unnamed-chunk-43-5.pdf}
\end{example}
